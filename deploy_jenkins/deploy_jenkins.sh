...
JENKINS_USE_NAMED_VOLUMES="${JENKINS_USE_NAMED_VOLUMES:-true}"
JENKINS_VOLUME_HOME="${JENKINS_VOLUME_HOME:-jenkins-home}"
...
deploy_jenkins() {
    ...
    if [[ "$JENKINS_USE_NAMED_VOLUMES" == "true" ]]; then
        log_info "使用 Docker 命名卷存储: $JENKINS_VOLUME_HOME"
    else
...
EOFPERMS
    ...
    if [[ "$JENKINS_USE_NAMED_VOLUMES" == "true" ]]; then
        volume_mount="-v $JENKINS_VOLUME_HOME:/var/jenkins_home"
        volume_display="$JENKINS_VOLUME_HOME (命名卷)"
    else
        volume_mount="-v $JENKINS_DATA_DIR:/var/jenkins_home"
        volume_display="$JENKINS_DATA_DIR (绑定挂载)"
    fi
