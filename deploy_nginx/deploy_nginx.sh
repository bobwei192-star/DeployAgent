...
NGINX_PORT_MANTISBT="${NGINX_PORT_MANTISBT:-18443}"
...
ensure_nginx_proxy() {
    ...
    local NGINX_PORT_MANTISBT="${NGINX_PORT_MANTISBT:-18443}"
...
server {
    ...
    location /jenkins/ {
        proxy_pass http://devopsagent-jenkins:8080;
...
NGINXEOF
    ...
    # 检测 MantisBT
    if docker ps --filter "name=devopsagent-mantisbt" --format "{{.Names}}" 2>/dev/null | grep -q .; then
        log_info "✓ 检测到 MantisBT 容器"
        detected_services+=("mantisbt")
        port_map_args="$port_map_args -p ${NGINX_BIND}:${NGINX_PORT_MANTISBT}:8443"

        log_info "创建/更新 mantisbt.conf..."
        cat > "$NGINX_CONF_D/mantisbt.conf" << 'NGINXEOF'
server {
    listen 8443 ssl;
    listen [::]:8443 ssl;
    ...
    location / {
        proxy_pass http://devopsagent-mantisbt:80;
        proxy_set_header Host $http_host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
deploy_nginx() {
    ...
    echo "  - 端口 MantisBT: $NGINX_BIND:$NGINX_PORT_MANTISBT -> 8443"
    ...
    docker run -d \
        ...
        -p "$NGINX_BIND:$NGINX_PORT_MANTISBT:8443" \
...
print_nginx_summary() {
    ...
    echo -e "  - MantisBT: ${CYAN}https://${bind_display}:$NGINX_PORT_MANTISBT${NC}"
